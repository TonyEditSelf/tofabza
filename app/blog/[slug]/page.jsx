...
import { Clock, ArrowLeft, Calendar as CalendarIcon, User as UserIcon } from "lucide-react";
...
export default async function BlogPostPage({ params }) {
  ...
  const relatedPosts = primaryCategoryId ? await getRelatedPosts(primaryCategoryId, post.slug) : [];
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="flex flex-wrap gap-2">
            {post.categoryDetails?.map(cat => (
              <Badge key={cat._id} variant="secondary" className="bg-accent/20 text-accent border-none">
                {cat.name}
              </Badge>
            ))}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-6 text-muted-foreground pt-4 border-t border-border/50">
            <div className="flex items-center gap-2">
              {post.authorDetails?.image ? (
                <img src={post.authorDetails.image} alt="Author" className="w-8 h-8 rounded-full" />
              ) : (
                <div className="w-8 h-8 rounded-full bg-accent/20 flex items-center justify-center text-accent">
                  <UserIcon className="w-4 h-4" />
                </div>
              )}
              <span className="font-medium text-foreground">{post.authorDetails?.name || "Tofabza Team"}</span>
            </div>
                  ...
                  <Card className="h-full flex flex-col bg-card border-border hover:border-accent/50 transition-colors duration-300">
                    ...
                    <CardContent className="flex-1 p-6 space-y-4">
                      ...
                      <div className="flex items-center text-sm text-muted-foreground mt-auto pt-4">
                        <Clock className="w-3 h-3 mr-1" /> {relatedPost.readingTime} min read
                      </div>
                    </CardContent>
                  </Card>
                </Link>
