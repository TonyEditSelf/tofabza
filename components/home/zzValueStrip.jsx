...
export default function ValueStrip() {
  return (
    <section className="py-16 border-y border-brand-500/10 bg-navy-900/40">
      <div className="container grid grid-cols-2 lg:grid-cols-4 gap-8">
        {VALUE_STRIP.map((v) => {
          const Icon = ICONS[v.icon];
          return (
            <div key={v.title} className="flex items-start gap-4 group">
              <div className="shrink-0 w-12 h-12 rounded-lg border border-brand-500/30 bg-navy-800/60 flex items-center justify-center group-hover:border-brand-500 transition-colors">
